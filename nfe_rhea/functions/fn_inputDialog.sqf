...
Text Input Example:
	0 spawn {
		(["Name", "What is your name?", "John"] call RHEA_fnc_inputDialog) params ["_status", "_text"];
		if (_status) then {
...
Combo Selection Example:
	0 spawn {
		(["Side", "Select a side:", "CIV", ["WEST", "EAST", "IND", "CIV"]] call RHEA_fnc_inputDialog) params ["_status", "_text"];
		if (_status) then {
...
createDialog "nfe_rhea_input";

private _dialog = uinamespace getvariable "RHEA_inputDialog";
...
waitUntil {!isNil {uiNamespace getVariable "RHEA_inputDialog_status"}};

private _status = uiNamespace getVariable "RHEA_inputDialog_status";
private _result = [_status];
uiNamespace setVariable ["RHEA_inputDialog_status", nil];
