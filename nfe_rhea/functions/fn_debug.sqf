...
switch _mode do {
	case "onLoad": {
		...
		for "_i" from 1 to 5 do {
			private _handler = compile format ["['updateWatch', [%1, %2] + _this] call RHEA_fnc_debug;", 1400 + _i, 1405 + _i];
	...
	case "updateWatch": {
		...
		private _status = _ctrlInput getVariable "nfe_rhea_debug_watchStatus";
