...
lbClear _listTasks;
{
	...
	_listTasks lbSetColor [_index, (_x call BIS_fnc_taskState) call {
		if (_this == "SUCCEEDED") exitWith { [0, 1, 0, 1] };
		if (_this == "FAILED") exitWith { [1, 0, 0, 1] };
		if (_this == "CANCELED") exitWith { [0.5, 0.5, 0.5, 1] };
		[1, 1, 1, 1]
	}];
